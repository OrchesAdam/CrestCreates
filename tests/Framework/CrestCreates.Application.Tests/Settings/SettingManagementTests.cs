...
public class SettingManagementTests
{
    ...
    public SettingManagementTests()
    {
        _settingDefinitionManager = new SettingDefinitionManager(
            [new CoreSettingDefinitionProvider(), new TestSettingDefinitionProvider()]);
    ...
    {
        await _settingManager.SetGlobalAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "GlobalName");
        await _settingManager.SetTenantAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "tenant-1", "TenantName");
        await _settingManager.SetUserAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "user-1", "UserName", "tenant-1");

        var provider = CreateSettingProvider("tenant-1", "user-1");

        var result = await provider.GetOrNullAsync(TestSettingDefinitionProvider.UserScopedDisplayName);
    ...
    public async Task RemoveOverride_ShouldFallbackToNextScope()
    {
        ...
        await _settingManager.RemoveUserAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "user-1", "tenant-1");
        var afterUserRemoval = await _settingValueResolver.ResolveAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "tenant-1", "user-1");
        afterUserRemoval.Value.Should().Be("TenantName");

        await _settingManager.RemoveTenantAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "tenant-1");
        var afterTenantRemoval = await _settingValueResolver.ResolveAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "tenant-1", "user-1");
        afterTenantRemoval.Value.Should().Be("GlobalName");

        await _settingManager.RemoveGlobalAsync(TestSettingDefinitionProvider.UserScopedDisplayName);
        var afterGlobalRemoval = await _settingValueResolver.ResolveAsync(TestSettingDefinitionProvider.UserScopedDisplayName, "tenant-1", "user-1");
        afterGlobalRemoval.Value.Should().Be("DefaultUserName");
        afterGlobalRemoval.Scope.Should().BeNull();
    }

    private sealed class TestSettingDefinitionProvider : ISettingDefinitionProvider
    {
        public const string UserScopedDisplayName = "Tests.UserScopedDisplayName";

        public void Define(SettingDefinitionContext context)
        {
            context.GetOrAddGroup("Tests", "Tests", "Test-only settings")
                .AddDefinition(
                    UserScopedDisplayName,
                    "User scoped display name",
                    "Test setting that supports all scopes",
                    "DefaultUserName",
                    SettingValueType.String,
                    false,
                    SettingScope.Global | SettingScope.Tenant | SettingScope.User);
        }
    }
