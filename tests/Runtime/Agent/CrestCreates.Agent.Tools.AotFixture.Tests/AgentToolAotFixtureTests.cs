...
namespace CrestCreates.Agent.Tools.AotFixture.Tests;

public sealed class AgentToolAotFixtureTests
{
    [Fact]
    public async Task Publish_native_aot_fixture_executes_governed_generated_tool_and_replay()
    {
        if (!OperatingSystem.IsLinux()
            || System.Runtime.InteropServices.RuntimeInformation.ProcessArchitecture
                != System.Runtime.InteropServices.Architecture.X64)
        {
            throw Xunit.Sdk.SkipException.ForSkip("The Phase 8f NativeAOT gate is pinned to linux-x64.");
        }

        var root = FindRepoRoot();
        var output = Path.Combine(Path.GetTempPath(), "crest-agent-tool-aot-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(output);
        try
        {
            var project = Path.Combine(
                root,
                "tests/Runtime/Agent/CrestCreates.Agent.Tools.AotFixture/CrestCreates.Agent.Tools.AotFixture.csproj");
            ...
            var executable = Path.Combine(output, "CrestCreates.Agent.Tools.AotFixture");
            ...
            execution.Output.Should().Contain("AGENT_TOOL_NATIVEAOT_PIPELINE_OK");
        }
        finally
        {
