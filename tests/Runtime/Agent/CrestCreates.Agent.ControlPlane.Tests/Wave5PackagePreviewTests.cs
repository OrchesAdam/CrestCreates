...
public class Wave5PackagePreviewTests : AgentControlPlaneTestBase
{
    ...
    [Fact]
    public async Task BuildPackageEvidencePreview_SameIdSameKindDifferentNamespace_ReuseRejected()
    {
        // When a descriptor is replaced with same Id + same Kind but different Namespace,
        // the VisibleDescriptorSetHash should differ (FullId includes Namespace).
        var options = AgentToolAuthorizationOptions.DevelopmentDefaults;
        var service = CreateService(options);
        ...
        // Step 2: Catalog replaces descriptor with same Id + same Kind but different Namespace
        var descV2 = new TestDescriptor { Namespace = "ns-b", Id = "desc-001", Kind = DescriptorKind.Event };
        DescriptorCatalogMock.Setup(c => c.GetAll()).Returns(new List<IDescriptor> { descV2 });
        DraftMaterializerMock.Setup(m => m.Materialize(draft, It.IsAny<IReadOnlyList<IDescriptor>>()))
            .Returns(DraftAbstractions.DescriptorDraftMaterializationResult.Success(
                new List<IDescriptor> { descV2 }.AsReadOnly()));

        // Step 3: Evidence preview should NOT reuse — FullId changed (ns-a.desc-001 → ns-b.desc-001)
        var evidence = await service.BuildPackageEvidencePreviewAsync(
            CreateContext("BuildPackageEvidencePreview"), "draft-001");
        evidence.Status.Should().Be(AgentToolResultStatus.Success);

        PackageBuilderMock.Invocations.Count.Should().BeGreaterThan(buildCountAfterPreview,
            "same Id + same Kind but different Namespace should force Path B rebuild");
    }

    [Fact]
    public async Task BuildPackageEvidencePreview_SameFullIdSameKindDifferentVersion_ReuseRejected()
    {
        // When a versioned descriptor is replaced with same FullId + same Kind but different Version,
        // the VisibleDescriptorSetHash should differ (Version is included in the fingerprint).
        var options = AgentToolAuthorizationOptions.DevelopmentDefaults;
        var service = CreateService(options);
        ...
        var descV1 = new TestVersionedDescriptor("test", "desc-001", version: 1);
        DescriptorCatalogMock.Setup(c => c.GetAll()).Returns(new List<IDescriptor> { descV1 });
        DraftMaterializerMock.Setup(m => m.Materialize(draft, It.IsAny<IReadOnlyList<IDescriptor>>()))
            .Returns(DraftAbstractions.DescriptorDraftMaterializationResult.Success(new List<IDescriptor> { descV1 }.AsReadOnly()));
        SetupPackageBuilderWithHashes();
        ...
        // Step 1: Create package preview with test/desc-001/v1
        var preview1 = await service.PreviewDescriptorPackageAsync(CreateContext("PreviewDescriptorPackage"), "draft-001");
        preview1.Status.Should().Be(AgentToolResultStatus.Success);
        var buildCountAfterPreview = PackageBuilderMock.Invocations.Count;

        // Step 2: Catalog replaces descriptor with same FullId + same Kind but different Version
        var descV2 = new TestVersionedDescriptor("test", "desc-001", version: 2);
        DescriptorCatalogMock.Setup(c => c.GetAll()).Returns(new List<IDescriptor> { descV2 });
        DraftMaterializerMock.Setup(m => m.Materialize(draft, It.IsAny<IReadOnlyList<IDescriptor>>()))
            .Returns(DraftAbstractions.DescriptorDraftMaterializationResult.Success(
                new List<IDescriptor> { descV2 }.AsReadOnly()));

        // Step 3: Evidence preview should NOT reuse — Version changed (1 → 2)
        var evidence = await service.BuildPackageEvidencePreviewAsync(
            CreateContext("BuildPackageEvidencePreview"), "draft-001");
        evidence.Status.Should().Be(AgentToolResultStatus.Success);

        PackageBuilderMock.Invocations.Count.Should().BeGreaterThan(buildCountAfterPreview,
            "same FullId + same Kind but different Version should force Path B rebuild");
    }
}
