...
namespace CrestCreates.Mcp.Memory.AotFixture.Tests;

public sealed class McpMemoryAotFixtureTests
{
    [Fact]
    public async Task Publish_native_aot_fixture_executes_mcp_memory_tool_pipeline()
    {
        if (!OperatingSystem.IsLinux()
            || System.Runtime.InteropServices.RuntimeInformation.ProcessArchitecture
                != System.Runtime.InteropServices.Architecture.X64)
            throw Xunit.Sdk.SkipException.ForSkip("The MCP Memory NativeAOT gate is pinned to linux-x64.");

        var root = FindRepoRoot();
        var output = Path.Combine(Path.GetTempPath(), "crest-mcp-memory-aot-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(output);
        try
        {
            var project = Path.Combine(root, "tests/Integrations/CrestCreates.Mcp.Memory.AotFixture/CrestCreates.Mcp.Memory.AotFixture.csproj");
            ...
            var executable = Path.Combine(output, "CrestCreates.Mcp.Memory.AotFixture");
