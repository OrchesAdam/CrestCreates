...
### Issue #87 — PR #96 and successor checkpoint (2026-09-14 evening)

PR #96 is draft, head 9a6f9c0be08f356026f9377192595d2b33e2f7fa.
Local CP 556/556 and Asset PostgreSQL E2E 12/12 passed. CI run 34836223554
is pending; check exact head before marking ready. Parent PR #95 is ready/unmerged.
Successor worktree asset-approved-host-87 contains uncommitted governance harness,
smoke test, csproj references and design notes. No successor PR exists.
The first harness build failed with 5 compile errors; Luna is repairing Draft
namespace alias, EnsureSuccess<T> class constraint and inline interaction ref.
Log /tmp/crest-approved-host-build.log. No harness test has run.
The smoke test is copied from /tmp/AssetApprovedInventoryHandoffAcceptanceTests.cs.
Review before composition: dependency graph still captures initial baseline; checked
inventory must be bound to its originating harness/tenant and rechecked before
catalog update. A checked flag alone is insufficient. Two-draft/Host/restart/live
model evidence remains absent.
Only InMemoryRuntimeActivationGate exists and does not install runtime inventory.
Do not claim production activation from status Activated or fixture handoff.
See docs/review/2026-09-14-phase10-gap-working-assessment.md for provisional
#88 directions; no deferred issue was activated or closed.
At 96% usage, one-time heartbeat crestcreates successfully created for
2026-09-14 23:56 Asia/Shanghai after actual reset 23:55:26. No reset card used.

Luna reported the five initial compile errors repaired and corrected the audit
Timestamp field; primary started a second build into the same log. Check its
completion before rerunning. The review task Interaction now reuses catalog data;
review whether its remaining local CC1001 suppression is unnecessary.

### Issue #87 — PR #96 verified (2026-09-14 23:56 wake)

CI 34836223554 completed successfully for exact head
9a6f9c0be08f356026f9377192595d2b33e2f7fa; primary rechecked PR head, marked
#96 ready and updated its evidence. No merge. The one-time heartbeat was deleted
after quota recovery (12% consumed). Second successor build exposed five compile
errors: smoke ActivationRequestStatus import; harness ActorRoles array conversion,
nested constructor accessibility and DependencyEdge namespace. Luna repairs are
in progress. No successor acceptance pass yet.

### Issue #87 — Executed authoring version gap (2026-09-15)

New harness single approval smoke passed 1/1. Subsequent two-draft acceptance
exposed a prohibited fixture workaround (manually replacing parser BaseVersion);
primary removed it. Existing 7g.v1 output DTO has no independent baseVersion and
parser sets both base/proposed from payload. The explicit Update baseVersion="1",
payload version=2 baseline now executes: 1 passed, 1 failed with actual BaseVersion
"2" instead of "1". Evidence: docs/review/2026-09-15-authoring-update-version-baseline.log.

Primary design docs/superpowers/plans/2026-09-15-authoring-update-base-version.md
adds optional explicit Update baseVersion to the existing parser contract. Missing
field preserves existing same-version behavior; no historical version inference.
Luna is implementing DTO/parser/prompt and focused tests. Native extension and
full validation remain required. No production parser repair is claimed yet.

Harness graph now reads current catalog snapshots and catalog addition rechecks
authoritative request/hash plus full descriptor identity and owner/tenant. These
paths executed before the two-draft version assertion failure, including foreign
receipt and changed receipt rejection. The second Workflow approval and Host
loading remain unexecuted. PR #96 stays verified/ready and unmodified.

### Resume after next five-hour window (2026-09-15)

One-time heartbeat crestcreates successfully created for 2026-09-15 04:57
Asia/Shanghai, after actual reset 04:56:30. No reset card redeemed.
PR #96 remains ready/unmerged with full CI success. Current successor files are
uncommitted; new UpdateBaseVersionParserTests.cs is not yet executed. The actual
version baseline is preserved in docs/review/2026-09-15-authoring-update-version-baseline.log
(1 pass, 1 failure at BaseVersion expected1/actual2). Luna production DTO/parser/
prompt patch is in progress; inspect working tree before testing. Next: focused
parser tests, full authoring, real two-draft acceptance without any parsed-envelope
patch, native fixture extension + publish/link/run, then full checks/PR. The
existing native minimal extension point is RunAgentAuthoringOptionalReferences
in ControlPlane.JsonContracts.AotFixture/Program.cs.
