...
namespace CrestCreates.CodeGenerator.EntityGenerator
{
    ...
    public class EntitySourceGenerator : IIncrementalGenerator
    {
        ...
        private void ExecuteGeneration(SourceProductionContext context, ImmutableArray<INamedTypeSymbol?> entityClasses)
        {
            ...
            foreach (var entityClass in entityClasses)
            {
                ...
                try
                {
                    ...
                    GenerateEntityFilterBuilder(context, entityClass);
                    GenerateEntitySortBuilder(context, entityClass);
                    GenerateEntityQueryRequest(context, entityClass);
                    GenerateEntityQueryExecutor(context, entityClass);
                    GenerateEntityQueryExtensions(context, entityClass);
        ...
        private void GenerateEntityQueryExecutor(SourceProductionContext context, INamedTypeSymbol entityClass)
        {
            ...
            sourceCode.AppendLine("using CrestCreates.Domain.Shared.DTOs;");
            sourceCode.AppendLine("using CrestCreates.Domain.Shared.Enums;");
            sourceCode.AppendLine("using CrestCreates.Domain.Shared.Query;");
            sourceCode.AppendLine($"using {namespaceName};");
            sourceCode.AppendLine();
            sourceCode.AppendLine($"namespace {targetNamespace}");
            sourceCode.AppendLine("{");
