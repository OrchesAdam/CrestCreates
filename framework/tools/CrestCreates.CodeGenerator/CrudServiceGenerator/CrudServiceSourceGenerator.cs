...
namespace CrestCreates.CodeGenerator.CrudServiceGenerator
{
    ...
    public class CrudServiceSourceGenerator : IIncrementalGenerator
    {
        ...
        private void ExecuteGeneration(SourceProductionContext context, ImmutableArray<(INamedTypeSymbol Symbol, bool GenerateAsBaseClass, bool IsUsingNewAttribute)?> entityClasses)
        {
            ...
            foreach (var entityInfo in entityClasses)
            {
                ...
                try
                {
                    ...
                    GenerateCrudServiceInterface(context, entityClass, entityName, namespaceName, idType);
        ...
        private void GenerateCrudServiceInterface(SourceProductionContext context, INamedTypeSymbol entityClass, string entityName, string namespaceName, string idType)
        {
            var builder = new StringBuilder();
            builder.AppendLine("// <auto-generated />");
            builder.AppendLine("using System;");
            ...
            builder.AppendLine("    /// </summary>");
            builder.AppendLine($"    public partial interface I{entityName}CrudService");
            builder.AppendLine("    {");
