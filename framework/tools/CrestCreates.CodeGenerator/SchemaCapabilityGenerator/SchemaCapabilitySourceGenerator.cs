        ...
        Compilation compilation)
    {
        ...
        if (hasHumanTask && humanTasks.Any(h => h != null))
        {
            ...
            foreach (var ht in humanTasks)
            {
                ...
                sb.AppendLine($"            Interaction = new VersionedDescriptorRef<IInteractionDescriptor>(\"{ht.FormId}\", {ht.FormVersion}),");
                if (ht.InputSchemaId != null)
                    sb.AppendLine($"            InputSchema = new VersionedDescriptorRef<SchemaDescriptor>(\"{ht.InputSchemaId}\", {ht.InputSchemaVersion!.Value}),");
                if (ht.OutputSchemaId != null)
                    sb.AppendLine($"            OutputSchema = new VersionedDescriptorRef<SchemaDescriptor>(\"{ht.OutputSchemaId}\", {ht.OutputSchemaVersion!.Value}),");
        ...
        if (hasWorkflow && workflows.Any(w => w != null))
        {
            ...
            foreach (var wf in workflows)
            {
                ...
                if (wf.VariableSchemaId != null)
                    sb.AppendLine($"            VariableSchema = new VersionedDescriptorRef<SchemaDescriptor>(\"{wf.VariableSchemaId}\", {wf.VariableSchemaVersion!.Value}),");
                if (wf.Steps.Count > 0)
                {
                    sb.AppendLine($"            Steps = new List<WorkflowStep>");
                    sb.AppendLine("            {");
                    foreach (var step in wf.Steps)
                    {
                        sb.AppendLine($"                new WorkflowStep");
                        sb.AppendLine("                {");
                        sb.AppendLine($"                    Id = \"{step.Id}\",");
                        sb.AppendLine($"                    Name = \"{step.Name}\",");
                        if (step.TargetType == "Capability" && step.CapabilityId != null)
                            sb.AppendLine($"                    Target = new CapabilityTarget {{ Capability = new VersionedDescriptorRef<IVersionedDescriptor>(\"{step.CapabilityId}\", 1) }},");
                        else if (step.TargetType == "HumanTask" && step.HumanTaskId != null)
                            sb.AppendLine($"                    Target = new HumanTaskTarget {{ HumanTask = new VersionedDescriptorRef<HumanTaskDescriptor>(\"{step.HumanTaskId}\", 1) }},");
                        else if (step.TargetType == "SubWorkflow" && step.SubWorkflowId != null)
                            sb.AppendLine($"                    Target = new SubWorkflowTarget {{ SubWorkflow = new VersionedDescriptorRef<WorkflowDescriptor>(\"{step.SubWorkflowId}\", 1) }},");
                        sb.AppendLine($"                    OnError = StepErrorBehavior.{step.OnError},");
                        sb.AppendLine("                },");
                    }
                    sb.AppendLine("            },");
                }
