...
public class DefaultCrestExceptionConverter : ICrestExceptionConverter
{
    ...
    public CrestExceptionConversionResult Convert(HttpContext context, Exception exception)
    {
        ...
        {
            CrestPermissionException permissionException => Create(context, CrestErrorCodes.AuthForbidden, 403, "没有权限执行当前操作。", permissionException.PermissionName),
            ...
            KeyNotFoundException keyNotFoundException => Create(context, CrestErrorCodes.EntityNotFound, 404, "资源不存在。", keyNotFoundException.Message),
            System.ComponentModel.DataAnnotations.ValidationException validationException => Create(context, CrestErrorCodes.ValidationFailed, 400, "数据验证失败。", validationException.Message),
            FluentValidation.ValidationException fluentValidationException => Create(context, CrestErrorCodes.ValidationFailed, 400, "数据验证失败。", string.Join("; ", fluentValidationException.Errors.Select(e => e.ErrorMessage))),
            ArgumentException => Create(context, CrestErrorCodes.InvalidArgument, 400, "请求参数错误。"),
            InvalidOperationException => Create(context, CrestErrorCodes.OperationInvalid, 400, "当前操作无效。"),
            _ => Create(context, CrestErrorCodes.InternalError, 500, "服务器内部错误。")
        };
    ...
    private CrestErrorResponse MapUnauthorized(HttpContext context)
    {
        ...
        return isAuthenticated
            ? Create(context, CrestErrorCodes.AuthForbidden, 403, "没有权限执行当前操作。")
            : Create(context, CrestErrorCodes.AuthUnauthorized, 401, "当前请求未认证。");
    }
