    ...
    NpgsqlDataSource dataSource) : IHumanTaskCompletionObligationPreflight
{
    public async ValueTask ValidateAsync(IReadOnlyList<HumanTaskCompletionObligationPolicyRegistration> policies, IReadOnlySet<string> activeConsumerIds, CancellationToken cancellationToken = default)
    {
        ...
        await using var command = new NpgsqlCommand($"select instance_id, status, state_json::text, required_consumer_ids_json::text from {table} where status in (0,1,4);", connection);
        await using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            var instanceId = reader.GetString(0);
            if (reader.GetInt32(1) == (int)HumanTaskInstanceStatus.CompletionDispatchFailed)
                throw new RuntimePersistenceContractException(RuntimePersistenceContractErrorCode.PersistedInvariantViolation, $"Legacy HumanTask '{instanceId}' is in CompletionDispatchFailed and must be explicitly reconciled before transactional outbox cutover.");
            var task = PostgreSqlRuntimeStoreSupport.Deserialize(reader.GetString(2), PostgreSqlRuntimeJsonSerializerContext.Default.HumanTaskInstance);
            task.RequiredCompletionConsumerIds = PostgreSqlRuntimeStoreSupport.Deserialize(reader.GetString(3), PostgreSqlRuntimeJsonSerializerContext.Default.StringArray);
