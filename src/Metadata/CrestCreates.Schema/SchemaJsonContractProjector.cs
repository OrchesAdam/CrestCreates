...
/// <summary>
/// Projects the framework's closed portable Schema subset to a canonical JSON
/// Schema object without applying a transport-specific envelope or error code.
/// </summary>
    ...
    private static (string Type, string? Format) ResolveToken(string token)
    {
        if (!SchemaScalarTypes.TryResolve(token, out var kind))
        {
            throw new SchemaJsonContractException(
                ...
                "Schema field type is not supported by JSON Schema projection.");
        }
        ...
        return kind switch
        {
            ...
            _ => throw new SchemaJsonContractException(
                SchemaJsonContractViolation.ScalarTypeUnsupported,
                "Schema field type is not supported by JSON Schema projection.")
        };
    }

    private static long EffectiveIntegralBound(
        double? configured,
        long minimum,
        long maximum,
        bool lower)
    {
        ...
        if (decimal.Truncate(value) != value || value < minimum || value > maximum)
        {
            throw new SchemaJsonContractException(
                ...
                "Schema integer constraint is invalid.");
        }
    ...
    private static void ValidateSchema(SchemaDescriptor schema)
    {
        if (schema.ValidationRules.Count > 0)
        {
            throw new SchemaJsonContractException(
                ...
                "Schema validation rules cannot be projected to JSON Schema.");
        }
        if (schema.References.Count > 0)
        {
            throw new SchemaJsonContractException(
                SchemaJsonContractViolation.ReferencesUnsupported,
                "Schema references cannot be projected to JSON Schema.");
        }
        ...
        foreach (var field in schema.Fields)
        {
            if (string.IsNullOrWhiteSpace(field.Name) || !names.Add(field.Name))
            {
                throw new SchemaJsonContractException(
                    ...
                    "Schema field identity is invalid.");
            }
            if (!string.IsNullOrEmpty(field.Pattern))
            {
                throw new SchemaJsonContractException(
                    SchemaJsonContractViolation.PatternUnsupported,
                    "Schema patterns are not portable to the supported JSON Schema subset.");
            }
            if (field.MinLength is < 0 || field.MaxLength is < 0 || field.MinLength > field.MaxLength)
            {
                throw new SchemaJsonContractException(
                    ...
                    "Schema length constraint is invalid.");
            }
                ...
                || field.MinValue > field.MaxValue)
            {
                throw new SchemaJsonContractException(
                    ...
                    "Schema numeric constraint is invalid.");
            }
            ...
            if (field.IsCollection && field.CollectionElementType is null)
            {
                throw new SchemaJsonContractException(
                    ...
                    "Collection element type is missing.");
            }
                ...
                || (type == "integer" || type == "number") && (field.MinLength.HasValue || field.MaxLength.HasValue))
            {
                throw new SchemaJsonContractException(
                    ...
                    "Schema constraint is not applicable to its field type.");
            }
        }
    }
}
