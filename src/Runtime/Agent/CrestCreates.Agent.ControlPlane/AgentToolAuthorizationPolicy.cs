...
/// <list type="bullet">
///   <item><see cref="AgentToolAuthorizationOptions.DevelopmentDefaults"/> is the true allow-all replacement (legacy <see cref="AllowAll"/> now maps to ExplicitPolicy)</item>
...
public sealed record AgentToolAuthorizationPolicy
{
    ...
    /// When converted via the legacy constructor, this maps to ExplicitPolicy with
    /// read-only defaults (not DevelopmentAllowAll), so mutating tools are still denied.
    /// For true allow-all behavior, use <see cref="AgentToolAuthorizationOptions.DevelopmentDefaults"/> directly.
